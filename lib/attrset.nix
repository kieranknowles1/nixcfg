    ...
    builtins.zipAttrsWith
      (
        ...
        if builtins.isAttrs first then
          deepMergeSets values # Recurse into nested sets
        else if builtins.isList first then
          builtins.concatLists values # Concatenate lists
        else
          first
      ) # We don't want to merge other types, so give the first value precedence
