...
}:
{
  ...
  config =
      ...
      mkBackupPair =
        ...
        let
          ...
          common =
            ...
            let
              finalPath =
                if cfgr.btrfs.useSnapshots then "${cfgr.btrfs.snapshotPath}/${tmpname}" else cfgr.source;
            in
            {
        ...
        [
          {
            inherit name;
            value = (common "local") // {
              repository = cfgr.destination.local;
            };
          }
          {
            name = "${name}-remote";
            value = (common "remote") // {
              repositoryFile = getSecret (mkRemotePath name);
            };
          }
        ];
    ...
    in
    {
      ...
      services.restic.backups = builtins.listToAttrs (builtins.concatMap mkBackupPair backups);
