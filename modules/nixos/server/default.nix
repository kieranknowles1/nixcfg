...
in
{
  ...
  options.custom.server =
    let
      ...
      vhostOpts = {
        root = mkHostOpt types.path "/path/to/html" "Absolute path to the root directory to be served.";
        proxyPort = mkHostOpt types.port 8080 "The port to proxy connections to.";
        proxySocket =
  ...
  config =
    let
      ...
      subhosts = lib.attrsets.mapAttrs' (name: subdomain: {
      ...
      mkVhost = subdomain: ssl: {
        locations."/" = {
          ...
          proxyPass =
            if subdomain.proxyPort != null then
              "http://localhost:${toString subdomain.proxyPort}"
            else if subdomain.proxySocket != null then
              "http://unix:${toString subdomain.proxySocket}"
            else
              null;
    ...
    in
    lib.mkIf cfg.enable {
      assertions = lib.attrsets.mapAttrsToList (name: subdomain: {
        assertion =
          builtins.length (
            builtins.filter isSet [
              subdomain.root
              subdomain.proxyPort
