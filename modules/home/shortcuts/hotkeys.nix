...
let
  ...
  mkDocs =
    bindings: visBinding:
    let
      bindingList = lib.lists.forEach bindings (
...
in
{
  options.custom.shortcuts.hotkeys =
    let
      inherit (lib)
        mkOption
        mkPackageOption
  ...
  config =
    ...
    lib.mkIf cfg.enable {
      ...
      custom.shortcuts.hotkeys.visualiser = {
        build.config = pkgs.writeText "keyboard-vis.json" (builtins.toJSON (map removeAction cfg.keys));
      };
      ...
      ++ (lib.optional cfg.visualiser.enable {
        inherit (cfg.visualiser.binding)
          key
          ctrl
          alt
          shift
