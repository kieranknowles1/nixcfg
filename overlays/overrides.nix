...
final: prev: {
  ...
  openmw =
    ...
    in
    if is049 then
      builtins.throw "OpenMW has been updated on nixpkgs, openmw-nix should no longer be needed"
    else
  ...
  homepage-dashboard =
    if prev.homepage-dashboard.version == "1.3.2" then
      prev.homepage-dashboard.overrideAttrs (_oldAttrs: rec {
        ...
        pnpmDeps = final.pnpm_10.fetchDeps {
          inherit (prev.homepage-dashboard)
            pname
            ;
      ...
      builtins.throw "Homepage-dashboard has been updated on nixpkgs, override no longer needed";
}
