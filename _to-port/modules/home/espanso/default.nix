{
  ...
  in
    lib.mkIf cfg.enable {
      # FIXME: This module is currently half NixOS, half Home Manager. Choose one or the other.
      # Probably NixOS since it needs a wrapper for Wayland
      # FIXME: Rainbow window flashes when Espanso starts. Probably a Wayland issue
      ...
      in
        packages
        // {
          "espanso/match/nix-globals.yml".source = yaml.generate "nix-globals.yml" {
            global_vars = let
